...
- `finance/public/js/app.js` — controller: cached-first boot, refresh, header/strip, popup (next two payments), overrides API, routing (`#/command|payoff|spending|projection|after|allocate|donuts|million|accounts|statements`, keys 1–9, 0, `r` = resync).
...
- Card statements (2026-09-11, from issuer apps) live in `plan.statements.cards` {due, dueDay, statement, minDue, minEst, payee}: Biz Gold 15th, Biz Plat 16th, 8018 20th, Amex Plat 21st, 3208 28th, Sapphire 22nd (closes 25th), Discover 14th (already a bill). `ENGINE.bills()` = `P.bills` + generated minimums (until the kill date); `ENGINE.statementBoard()` feeds the Statements page (`#/statements`, key 0). Rule: due dates ≠ interest; carrying cards accrue daily, so kill whole cards and pay only minimums on the rest; living from checking.
