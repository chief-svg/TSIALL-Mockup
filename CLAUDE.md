...
- `finance/public/js/app.js` — controller: cached-first boot, refresh, header/strip, popup (next two payments), overrides API, routing (`#/command|payoff|spending|projection|after|million|accounts`, keys 1–7, `r` = resync).
...
- PocketSmith MCP tools work from a Claude Code session here; connector calls from the phone viewers failed until the display-name fix (unconfirmed as of 2026-09-07).
