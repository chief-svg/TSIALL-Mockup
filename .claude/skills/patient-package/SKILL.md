...
3. Write `patients/<id>-note.md` with: `# Patient <initials>: Consult Note`, a one-line `> ` note explaining the blocks, then sections 1 to 10 in this order, each note section as a fenced ``` block: 1 Reason for visit and HPI; 2 Oncologic history and timeline; 3 Past medical, surgical, medications, allergies; 4 Social and family history; 5 Review of systems; 6 Vitals and physical examination; 7 Data reviewed (each report with its date and author); 8 Assessment (five numbered items, each preceded by a `::: cue Add if discussed` block listing what can be added; item 5 is records reviewed and trial status); 9 Plan (paragraphs A to K from the template, each preceded by a `::: cue` block naming the options, lists, or modules that may be added); 10 Before signing (plain paragraph); 11 Case Log fields (template Part 8 filled for this patient, as a fenced block, marked as tracker entry, not note text). Use the surgeon's voice and the five anchor sentences verbatim. Leave `[ ]` for unknowns. Include only modules that may apply, marked with their ids.
