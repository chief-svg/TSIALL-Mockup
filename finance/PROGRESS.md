...
## Decisions
- 2026-09-15: two feed quirks fixed in the engine: (1) PocketSmith filed the $10,000 reimbursement (PRMA MSO … DES:CORP PAY) as a *transfer* → payroll-style credits into checking now classify as income (`RX.payroll`); (2) BofA's ledger balance excludes pending debits (the $9,500 ACH hold) while Citi includes pending charges → cash accounts are adjusted by their pending transactions (`pendingAdj`, shown on the Cash tile).
- 2026-09-15: 8018 Aug statement paid in full ($9,500 Sep 11 + $3,963.71 Sep 15) — grace holds. Open charges as of Sep 15: $8,743.39* (incl. $627 car care, $2,535 flight, pending) → `openChargesAsOf` added so the first statement bill only adds living after that date; Oct 20 statement ≈ $10,100*. Sep 27 Amex Plat cut to $300; downstream amounts re-solved (Sapphire kill $11,500 Dec 27, IRS kill $7,350 Jan 27). Interest ≈ $4,550*.
- 2026-09-12: **Read-only snapshot build** for sharing with Jessica (no Claude account needed): `node hosted/build.js --static data.json` → `hosted/dist/snapshot.html` (gitignored — contains transactions). `window.STATIC_DATA = {at, payload, overrides, snapshots}` puts the app in `mode: 'static'`: no connector, Resync hidden, manual-balance UI hidden, badge “Snapshot · read-only”. Data assembled in the scratchpad from a fresh MCP pull (`static-data.json`). Published as a separate artifact.
- 2026-09-12: **Manual-balance prompt** for accounts PocketSmith can't see (`extraDebts`: IRS, Discover, Jessica's Citi 1991). The load popup lists them with an input prefilled with the last balance; rows older than 7 days are flagged amber. Save → `APP.setOverrides` (manual, dated today) → rebuild.
- 2026-09-12: **Jessica's Citi Diamond Preferred …1991** added (`extraDebts` id `jess1991`, $5,786.91, min $58 due the 1st, closes the 3rd). Statement shows a true introductory 0% (“Introductory Rate Expires 09/28/26”, $0 interest YTD) — not deferred interest, nothing retroactive; go-to APR assumed 27.99%* (`standardApr`) until confirmed. Engine now applies `promoEnds`/`standardApr` per day (`aprOn`) in sims, near-term and the statement board. Slotted in rate order after Biz Gold: $5,600 Nov 27, tail Dec 15; Sapphire kill moves to Dec 27 ($10,900*), IRS kill Jan 27. Interest ≈ $4,550*. Plan-start debt restated to $149,361.
