...
window.APP = (() => {
  ...
  function renderStrip() {
    ...
    el.innerHTML = `
      ...
      <div class="stat"><div class="eyebrow">Spend today · tomorrow</div><div class="v"><span class="${liv.todayLeft > 0 ? (liv.todayLeft < PLAN.living.perDay * 0.5 ? 'amber' : 'pos') : 'neg'}">${F.money(liv.todayLeft)}</span> <span class="muted" style="font-size:15px">· ${F.money(liv.tomorrowLeft)}</span></div><div class="s">${liv.todayLeft > 0 ? `left today · spent ${F.money(liv.spentToday)} so far` : `over pace by ${F.money(liv.overBy)}`} · tomorrow if nothing more today</div></div>
