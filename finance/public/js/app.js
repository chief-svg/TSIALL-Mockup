...
window.APP = (() => {
  ...
  function renderStrip() {
    ...
    el.innerHTML = `
      ...
      <div class="stat"><div class="eyebrow">Cash</div><div class="v">${F.money(t.cash)}</div><div class="s">BofA checking${(() => { const c = state.ctx.byId[PLAN.checkingId]; return c && c.pendingAdj ? ` · ${F.money(c.feedBalance)} feed ${c.pendingAdj < 0 ? '−' : '+'} ${F.money(Math.abs(c.pendingAdj))} pending${UI.ast()}` : ''; })()}${ovCount ? ` · <a href="#/accounts" class="blue">${ovCount} manual balance${ovCount > 1 ? 's' : ''}</a>` : ''}</div></div>
      ...
      <div class="stat"><div class="eyebrow">Spend today · tomorrow</div><div class="v"><span class="${liv.todayLeft > 0 ? (liv.todayLeft < liv.per * 0.5 ? 'amber' : 'pos') : 'neg'}">${F.money(liv.todayLeft)}</span> <span class="muted" style="font-size:15px">· ${F.money(liv.tomorrowLeft)}</span></div><div class="s">${liv.todayLeft > 0 ? `left today · spent ${F.money(liv.spentToday)} so far` : `over pace by ${F.money(liv.overBy)}`} · tomorrow if nothing more today</div></div>
  ...
  function showUpcoming() {
    ...
    bg.innerHTML = `<div class="modal">
      ...
      <div style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid var(--hair)"><div><span class="num ${state.d.living.todayLeft > 0 ? 'pos' : 'neg'}" style="font-size:24px">${F.money(state.d.living.todayLeft)}</span> <span class="small muted">${state.d.living.todayLeft > 0 ? 'left today' : 'over pace by ' + F.money(state.d.living.overBy)}</span> <span class="num muted" style="font-size:18px;margin-left:8px">${F.money(state.d.living.tomorrowLeft)}</span> <span class="small muted">tomorrow</span></div><div class="xs muted right">${F.money(PLAN.living.monthly)}/mo = ${F.money(state.d.living.per)}/day · ${F.money(Math.max(0, state.d.living.perDayInclToday))}/day for the rest of ${F.fmtMonth(state.d.living.from)}</div></div>
