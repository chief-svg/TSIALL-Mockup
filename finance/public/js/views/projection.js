...
VIEWS.projection = {
  render(S) {
    const { ctx, d } = S; const pr = d.proj; const t = ctx.totals;
  ...
  mount(S) {
    const pr = S.d.proj; const pts = pr.pts;
    CH.make(document.getElementById('c-proj'), {
      ...
      options: { maintainAspectRatio: false, interaction: { mode: 'nearest', axis: 'x', intersect: false },
        scales: { x: { type: 'linear', min: 0, max: pts[pts.length - 1].x, grid: { display: false }, border: { display: false }, ticks: { stepSize: 30, autoSkip: false, maxRotation: 0, callback: v => { const p = pts.find(q => Math.abs(q.x - v) < 8); return p ? p.label : ''; } } }, y: CH.yMoney() },
