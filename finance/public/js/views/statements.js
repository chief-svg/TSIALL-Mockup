  ...
  render(S) {
    ...
    const rows = B.cards.map(c => {
      ...
      return `<tr class="${c.dead ? 'dead' : ''}">
        ...
        <td class="num">${min}</td>
        <td class="num neg">${F.money(-c.bal)}<span class="sub">${c.perDay > 0 ? `${((a.promoEnds && ctx.today > a.promoEnds ? a.standardApr : a.apr) * 100).toFixed(2)}% · ${F.money(c.perDay)}/day` : a.promoEnds && a.standardApr ? `0% to ${F.fmtDate(a.promoEnds, { year: false })}, then ${(a.standardApr * 100).toFixed(2)}%${UI.ast()}` : '0%'}</span></td>
        <td>${status}</td>
